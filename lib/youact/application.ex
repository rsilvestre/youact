defmodule Youact.Application do
  @moduledoc false
  use Application
  ...
  def start(_type, _args) do
    ...
    cache_config = Application.get_env(:youact, :cache, [])

    children = [
      # Pass the app_name to allow YouCache to read configuration from :youact app
      {Youact.Cache, [app_name: :youact] ++ cache_config}
    ]

    opts = [strategy: :one_for_one, name: Youact.Supervisor]
  ...
  defp ensure_cache_directories do
    ...
    cache_backends = Application.get_env(:youact, :cache_backends, %{})
